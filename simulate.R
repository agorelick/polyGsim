## start with a random chronological phylogeny, generate poly-G data for it, try to recover it.
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# simulate a random coalescence tree which X-axis is time
# then use this to simulate data for the appropriate number of cell divs
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

random_number_generations <- function(starting_cells=1, s=0.004, k=3, max_gens=1e4, max_cells=1e12) { 
...
sim_coalescence <- function(n_samples,n_samples_is_average=T,tip.label='normal') {  
...
gens <- random_number_generations()
max_gens <- gens$generations
...
gm <- matrix(nrow=max_gens,ncol=n_samples)
gm[is.na(gm)] <- as.integer(0)
colnames(gm) <- samples
for(g in 1:(max_gens-1)) {
...
simulate_mutations_in_levels_for_gen <- function(n_levels, n_markers) {
    tmp <- matrix(nrow=n_levels,ncol=n_markers*2)
    tmp[is.na(tmp)] <- 0
    tmp <- rcpp_mutate_length_matrix(tmp, 1e-2, 1)
    tmp
}
...
for(g in 1:limit) {
    ...
    tmp <- simulate_mutations_in_levels_for_gen(nl, 58)
    i <- 1
    for(i in 1:nl) { 
        lev <- unique_levels[i]
